...
-- The first parameter is the pitch list, the second is a number which refers to how many pitchs are grouped in a guess.
...
-- Calculate all the results of guesslist with target, then put all the results in a list and return that list
-- [String] ------------ target
-- [[String]] ---------- guesslist
...
-- Count the occurence of each result in the resultlist and return the largest number
-- [(Int,Int,Int)] ----- resultList
-- Int ----------------- largest one
...
-- This function works with countList together to get the largest number of occurence in result list
-- [((Int, Int, Int),Int)]----- resultList
...
-- Use max-min search to find the BestGuess as the next guess. General idea is to pick up a guess ( as potential target) in the guesslist and calculate the feedback of the potential target with every guess in the guesslist. Then group the the guess base on whether the feedback is the same. Count the number of the largest group. THIS IS THE MAXIMUM NUMBER. Then pick up every guess as potential target and repeat the process above. And then get the MAXIMUM NUMBER for each guess. Finally, the guess which has the minimal MAXIMUM NUMBER would be the BEST GUESS for next guess.
-- [[String]] ----------- 'to' list. would be empty in the beginning. A guess which has been caculated in 'from' list will be moved in this list
-- [[String]] ----------- 'from' list. would contain all guesses in the beginning. A guess which has been caculated will be moving out.
bestGuess :: [[String]]->[[String]] -> Int -> [String] -> [String]
bestGuess _ [] _ tempGuess = tempGuess
bestGuess to (a:as) tempCount tempGuess = 
    let aResult = countList (resultCategory a (to++as)) in 
...
-- Initial guess would be ["A1","B1","C2"] following by HINT 4. Initial GameState will be the whole guess list containing all the 1330 potential guesses.
initialGuess :: ([String],GameState)
initialGuess = (["A1","B1","C2"],GameState guessList)


-- Firstly, it will remove all the guesses which has different feedback with the previous guess from previous guessing feedback in the GameState. Then use the max-min search to get the BestGuess as next guess.
...
nextGuess (currentGuess,GameState (a:as) ) (r1,r2,r3) =
    ...
    (bestGuess [] newstate 1300 [],GameState newstate)

-- Caculate the feedback of every potential guess in guesslist with previous guess. If the feedback is different from the previous feedback, then remove the guess in the guesslist.(Hint 2)
-- GameState ----------- 'from' list. Would contain all guesses in the beginning. A guess which has different feedback from previous feedback will be considered as a Valid one and will be removed. Otherwise it will be add to to list
-- GameState ----------- 'to' list. would be empty in the beginning. A valid guess will be added to this.
